...
(defun from-string-to-list (sss)
  (let ((L (read-from-string 
    ...
    L))
(defvar botStrList nil)
(defvar botSymList nil)
(defvar valStrList nil)
(defvar valSymList nil)

(defun countBotStillHasChip(vv)
(let ((cntt 0))
    (dolist (bb botStrList)
        (when (> (length (getHashh bb "CHIPS")) 0)
            (setq cntt (1+ cntt))
        )
    )
cntt
...
(defun printBotHash(vv)
(let ()
    (dolist (bb botStrList)
        (when (> (length (getHashh bb "CHIPS")) 0)
            (format t "~% ~A ~A ~A ~A" bb (getHashh bb "LOW") (getHashh bb "HIGH") (getHashh bb "CHIPS"))
        )
    )
))

(defun getHashh(k1 k2)
(let (s1 s2)
    (setq s1 (intern k1))
    (setq s2 (intern k2))
    (get s1 s2)
...
(defun readValue(wList)
(let (n0 n1 n4 n5 valN dest)
    (setq n0 (string (nth 0 wList)))
    (setq n1 (write-to-string (nth 1 wList)))
    (setq n4 (string (nth 4 wList)))
    (setq n5 (write-to-string (nth 5 wList)))
    (setq valN (concatenate 'string n0 n1))
    (setq dest (concatenate 'string n4 n5))
    (setHashh valN "DESTINATION" dest)
    (setHashh valN "VALUE" (nth 1 wList))
    (when (not (member valN valStrList))
        (setq valStrList (cons valN valStrList))
        (setq valSymList (cons (intern valN) valSymList))
    )
))

(defun readBot(wList)
(let (n0 n1 n5 n6 n10 n11 botN dest1 dest2)
    (setq n0 (string (nth 0 wList)))
    (setq n1 (write-to-string (nth 1 wList)))
    (setq n5 (string (nth 5 wList)))
    (setq n6 (write-to-string (nth 6 wList)))
    (setq n10 (string (nth 10 wList)))
    (setq n11 (write-to-string (nth 11 wList)))
    (setq botN (concatenate 'string n0 n1))
    (setq dest1 (concatenate 'string n5 n6))
    (setq dest2 (concatenate 'string n10 n11))
    (setHashh botN "LOW" dest1)
    (setHashh botN "HIGH" dest2)
    (setHashh botN "CHIPS" nil)
    (when (not (member botN botStrList))
        (setq botStrList (cons botN botStrList))
        (setq botSymList (cons (intern botN) botSymList))
    )
))

(defun readInstruction(instList)
(let (lline wList wordList n0 n1 n5 n6 n10 n11 botNum botORvalue botORvalueNum botOutLow botOutHigh botOutLowNum botOutHighNum valToBotNum)
  (dolist (ii (loop for ii from 0 to (- (length instList) 1) collect ii))
    (setq lline (nth ii instList))
    (setq wordList (from-string-to-list lline))
    (if (= (length wordList) 12)
        (readBot wordList)
        (readValue wordList)
    )
  )
...
(defun processValue(vv)
(let (value dest)
    (dolist (bb valStrList)
        (setq value (getHashh bb "VALUE"))
        (setq dest (getHashh bb "DESTINATION"))
        (setHashh dest "CHIPS" (sort (cons value (getHashh dest "CHIPS")) #'<))
    )
))
(defvar FoundBot nil)
(defun processBot(bbot)
(let (chips lowDest highDest (FFOUND nil))
    (setq chips (getHashh bbot "CHIPS"))
    (when (= (length chips) 2)
        (when (and (= (car chips) 17) (= (cadr chips) 61)) (setq FoundBot bbot))
        (setq lowDest (getHashh bbot "LOW"))
        (setq highDest (getHashh bbot "HIGH"))
        (setHashh lowDest "CHIPS" (sort (cons (car chips) (getHashh lowDest "CHIPS")) #'<))
        (setHashh highDest "CHIPS" (sort (cons (cadr chips) (getHashh highDest "CHIPS")) #'<))
        (setHashh bbot "CHIPS" nil)
    )
))


(defun processBotInst(vv)
(let ( rett (FOUNDD nil) (cntLoop 0) chipCnt)
    (setq chipCnt (countBotStillHasChip t))
    (loop while (> chipCnt 0) do
        (dolist (bb botStrList)
            (setq rett (processBot bb))
        )
        (setq chipCnt (countBotStillHasChip t))
        (setq cntLoop (1+ cntLoop))
    )
))

(defun main(vv)
(let (foundBot)
    (readInstruction instList)
    (processValue t)
    (processBotInst t)
    ;(printBotHash t)
    (format t "~% Day10, Part 1: The number of the BOT is ~A" FoundBot)
    (format t "~% Day10, Part 2: ~D is the value after multiplied together the values of one chip in each of outputs 0, 1, and 2." (* (* (car (getHashh "OUTPUT0" "CHIPS")) (car (getHashh "OUTPUT1" "CHIPS"))) (car (getHashh "OUTPUT2" "CHIPS"))))
))
